...
# --- MANDATORY PYTHON 3.14+ INITIALIZATION BLOCK (MUST BE LINES 1-13) ---
try:
...
# Now it is completely safe to load external framework engines
...
# --- HTTP WEB PORT RECEPTACLE (Cleans Log Head Hooks) ---
...
# --- INSTANTIATE ENVIRONMENTS & RESTRUCK PASSPHRASE ---
...
bot = Client(
    "omega_media_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN
)
...
async def save_profile_update(user_id, system_key, value_payload):
    await user_profiles.update_one(
        {"user_id": user_id}, {"$set": {system_key: value_payload}}, upsert=True
    )


# --- BANDWIDTH SPEED TRACK PANEL ---
async def progress_bar_handler(
    current, total, status_message, action_text, start_time
):
    now = time.time()
    diff = now - start_time
...
@bot.on_message(
    filters.command("setcaption") & filters.private & is_authorized_user()
)
async def set_caption(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            "❌ **Usage:** `/setcaption Write your text here`"
        )
...
@bot.on_message(
    filters.command("clearcaption") & filters.private & is_authorized_user()
)
async def clear_caption(client: Client, message: Message):
    ...
    await message.reply_text(
        "✅ **Custom template cleared. Automated metadata active.**"
    )
...
async def set_name(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            "❌ **Usage:** `/setname filename.mp4` (Include extension!)"
        )
    target_name = message.text.split(None, 1)[1]
    await save_profile_update(message.from_user.id, "temp_rename", target_name)
    await message.reply_text(
        f"✏️ **Next queued file title targeted as:** `{target_name}`"
    )


@bot.on_message(
    filters.command("setchannel") & filters.private & is_authorized_user()
)
async def set_channel(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            "❌ **Usage:** `/setchannel @mychannelusername`"
        )
...
async def process_photo_preset(client: Client, message: Message):
    ...
    await message.reply_text(
        f"✅ **Artwork saved permanently! Slot ID:** `{preset_key}`"
    )


@bot.on_message(
    (filters.video | filters.document) & filters.private & is_authorized_user()
)
